...
### 2026-05-05 — PR #86 review (MAF v1.2.2 upgrade by Bishop)
- **Verdict:** APPROVE.
- Verified all current MAF call sites adapted: factory.py (Agent direct import, positional client, default_options={"response_format": ...}), pipeline.py (orchestrations namespace + AgentResponse fallback), poc/maf_poc/orchestrator.py + stub_agents.py (new constructor + _resolve_workflow_output helper), tests + helpers updated.
- Confirmed Ash's flag on reconciler.py/analyzer.py is moot — those files don't exist yet (Sprint 1 backlog, correctly out of scope).
- Pinning gent-framework>=1.2.2,<2.0 and adding gent-framework-azure-cosmos>=1.2.2,<2.0 aligns with Sprint 1 CosmosCheckpointStorage plans.
- Logged non-blocking follow-ups for Sprint 1: (1) consolidate _resolve_workflow_output and the inline normalizer in _run_workflow into one shared helper; (2) prefer keyword client= constructor style for resilience; (3) consider tightening if text is not None to if text: if empty-string regressions appear; (4) promote Bishop's MAF v1.2.2 pattern note into team-wide standard during inbox merge.
- Architectural pattern to remember: terminal workflow event normalization for MAF v1.2.2 is .text → .messages[-1].content → raw data. This shape is now codified and should be the single source of truth going forward.
